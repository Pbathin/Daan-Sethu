...
export default function DonateGadgets() {
    ...
    const saveGadgetList = async (imageUrl) => {
        await setDoc(doc(db, 'GadgetList', Date.now().toString()), {
    ...
    return (
        <ScrollView style={{ padding: 10 }}>
            <Text style={{ fontFamily: 'outfit', color: "gray" }}>
                Fill all the details to donate gadgets
            </Text>
            ...
            <View>
                <TextInput placeholder='Gadget Name'
                    onChangeText={(v) => setGadgetName(v)}
                    style={styles.TxtIp}
                />
                ...
                <TouchableOpacity
                    disabled={loading}
                    style={styles.btn}
                    onPress={() => onDonateGadgets()}>
                    {loading ? (
                        <ActivityIndicator size={'large'} color={'#fff'} />
                    ) : (
                        <Text style={styles.btnTxt}>Donate Gadgets</Text>
                    )}
                </TouchableOpacity>
            </View>
