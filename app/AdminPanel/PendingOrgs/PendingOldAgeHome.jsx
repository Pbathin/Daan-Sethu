...
export default function PendingOldAgeHome() {
    const [PendingOldAgeHome, setPendingOldAgeHome] = useState([]);
    const [loading, setLoading] = useState(true);  // Add loading state
    const navigation = useNavigation();

    useEffect(() => {
        navigation.setOptions({
            headerTitle: "Pending Old Age Home",
            headerShown: true,
            headerStyle: {
    ...
    useEffect(() => {
        const GetPendingOldAgeHome = async () => {
            setLoading(true);  // Start loading
            const q = query(collection(db, 'pendingOrgList'));
            const querySnapshot = await getDocs(q);
            const filteredOrgs = querySnapshot.docs
                .map(doc => ({ ...doc.data(), id: doc.id }))
                .filter(org => org.orgType === "Old Age Home");  // Filter for Old Age Home only

            setPendingOldAgeHome(filteredOrgs);
            setLoading(false);  // End loading
        };

        GetPendingOldAgeHome();
    }, []);
    ...
    const verifyOrganization = async (org) => {
        try {
            ...
            setPendingOldAgeHome(PendingOldAgeHome.filter((item) => item.id !== org.id));
    ...
    return (
        <View style={styles.container}>
            {loading ? (  // Show loading indicator
                ...
                PendingOldAgeHome.length === 0 ? (
                    // Show this when the list is empty
                    <Text style={styles.noPendingText}>No pending verifications of old age home</Text>
                ) : (
                    <FlatList
                        data={PendingOldAgeHome}
                        vertical={true}
